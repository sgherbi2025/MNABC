...
def abc_ant(evaluate, bounds, f_opt, config, max_fes, log_fes, seed):
    ...
    fes    = sn
    def tof(o): return 1.0/(1.0+o) if o>=0 else 1.0+abs(o)
...
def run_all(config):
    ...
    for a, fi, ri in tqdm(tasks, desc='Running'):
        ...
        all_r.append(res)
    print(f'Done. {len(all_r)} results computed.')
    return all_r
...
if not wdf.empty:
    ...
    print('W/D/L  (+ = ABC-ANT better):')
    print("No results to display (no comparison performed).")
...
if len(algos) >= 3:
    ...
    print(f"Friedman test not possible: at least 3 algorithms required, got {len(algos)}.")
