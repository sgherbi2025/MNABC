print("ASABC — Adaptive Exploration-Exploitation Switching ABC")
print("Reference: Zhang et al., Expert Systems with Applications 318 (2026) 131979")
print("Benchmarks: CEC 2017 (D=10,30,50,100) | CEC 2022 (D=10,20)")
print("Protocol: 51 runs (CEC2017) / 30 runs (CEC2022)")
print("Output: mean error, std, Friedman rank, Wilcoxon vs AutoABC & EABC-AS, convergence curves, LaTeX tables")
print("Instructions: pip install opfunu | set DIM and SUITE in section 3 | run section 7 for tables")
...
def run_experiments():
    ...
    for fid in tqdm(FUNC_IDS, desc='Functions'):
        ...
        for run in tqdm(range(N_RUNS), desc=f'  f{fid}', leave=False):
            algo = ASABC(
                func     = shifted_func,
                dim      = DIM,
                bounds   = BOUNDS,
